...
class MvnArchetypeUtil {
    ...
    getGenerateCmdArgs(projectName, options, logger = nullLogger) {
        ...
        const currentArtifactId = params.get('-DartifactId');
        const artifactId = this.sanitizeArtifactId(currentArtifactId);
        if (artifactId !== currentArtifactId) {
            logger.warn(`[cds] The Maven artifact id '${currentArtifactId}' is incompatible with Cloud Foundry, using '${artifactId}' instead.`);
            params.set(`-DartifactId`, artifactId);
        }
